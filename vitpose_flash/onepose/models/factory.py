...
import vitpose_flash.onepose.models.vitpose as vitpose
from PIL import Image
import torch_tensorrt


from vitpose_flash.onepose.utils import read_cfg, download_weights
from vitpose_flash.onepose.transforms import ComposeTransforms, BGR2RGB, TopDownAffine, ToTensor, NormalizeTensor, _box2cs
from vitpose_flash.onepose.functional import keypoints_from_heatmaps
import os
import pathlib
...
class Model(nn.Module):
    def __init__(self,
                model_name: str = 'ViTPose_huge_simple_coco',
                compile_flag: bool = False,
                compile_batch_size: int = 1,
                compiled_checkpoint_path: Optional[str] = None,
                **kwargs) -> None:
        ...
        self.model = vitpose.ViTPose(self.model_cfg.model, **kwargs)
        ...
        if compile_flag:
            input_shape = (compile_batch_size, 3, *self.model_cfg.model['backbone']['img_size'])
            print("Compiling using TensorRT")
            self.model.backbone = compile_tensorrt(self.model.backbone.cuda(), input_shape,
                                            compiled_checkpoint_path,
                                            self.model.dtype
                                        )
...
def create_model(model_name: str = 'ViTPose_huge_simple_coco', **kwargs) -> Model:
...
def compile_tensorrt(model, input_shape, compiled_checkpoint_path, dtype):
    """ Compiles the model into TensorRT format and saves the compiled
    checkpoint if specified.

    -------------------------------------------------------------------
    Args:
        model: The PyTorch model to be compiled.
        input_shape: Shape of the input tensor (batch size, channels, height, width).
        compiled_checkpoint_path: Path to save or load the compiled model checkpoint.
        dtype: Data type for the model (e.g., torch.float32, torch.float16).

    Returns:
        Compiled TensorRT model.
    """
    compile_flag = True
    inputs = [torch.rand(*input_shape, dtype=dtype).cuda()]
    if compiled_checkpoint_path is not None:
        # Check if a pre-compiled model is available at the path
        if os.path.isfile(compiled_checkpoint_path):
            # Load the precompiled model
            compiled_model = torch_tensorrt.load(compiled_checkpoint_path)
            exp_program2 = torch.export.export(model, tuple(inputs))
            model = torch_tensorrt.dynamo.refit_module_weights(
                compiled_module=compiled_model,
    ...
    if compile_flag:
        # Compile the model
        exp_pgm = torch_tensorrt.dynamo.trace(model, inputs)
        ...
        if compiled_checkpoint_path is not None:
            ...
            torch_tensorrt.save(model, compiled_checkpoint_path, inputs=inputs)
